...
이 배경 설명은 Google C++ 스타일 가이드가 단순한 취향 모음이 아니라 복잡한 C++ 코드를 오래 유지하기 위한 약속이라는 점을 알려줍니다. C++는 강력하지만 그만큼 위험한 선택지도 많기 때문에, 팀 전체가 같은 기준으로 읽기 쉬운 코드를 작성해야 합니다.

쉽게 말해 이 문서는 "내가 지금 편하게 쓰는 코드"보다 "다른 사람이 나중에 안전하게 읽고 고칠 수 있는 코드"를 우선하는 기준입니다.
