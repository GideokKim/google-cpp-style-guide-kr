...
`포인터/참조 표현식과 타입`은 해당 C++ 기능을 사용할 때 명확성, 안전성, 유지보수성을 함께 판단하라는 규칙입니다. 기능의 존재보다 독자가 비용 없이 의미를 이해할 수 있는지가 더 중요합니다.
...
쉽게 말해, `포인터/참조 표현식과 타입` 규칙은 독자가 코드를 읽으면서 추측해야 하는 부분을 줄이기 위한 안전장치입니다. 작성자는 배경을 알고 있지만, 리뷰어와 미래의 유지보수자는 이름·선언·형식에 남은 단서만 보고 판단합니다.
