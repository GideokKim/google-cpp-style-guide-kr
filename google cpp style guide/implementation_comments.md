...
### 핵심: 까다로운 곳에 주석을, 자명한 곳엔 주석 금지

까다롭거나 복잡하거나 중요한 코드 앞에는 주석을 답니다. 반대로 **코드를 그대로 읽어 주는** 주석(자명한 것)은 쓰지 마세요 — 더 높은 수준의 "왜"를 설명하거나, 코드를 자기 설명적으로 바꾸세요.
...
`CalculateProduct(values, 7, false, nullptr)`처럼 의미를 알 수 없는 인수가 늘어나면, 주석에 앞서 다음을 고려하세요.

- 리터럴 상수 → **명명된 상수**
- `bool` 인수 → **enum** (`kDontUseCache`)
- 옵션이 많으면 → **옵션 구조체/클래스**로 묶어 전달
- 큰 중첩 표현식 → **명명된 변수**
- 최후의 수단으로만 → 인수 옆 인라인 주석(`/*completion_callback=*/nullptr`)
