...
`세로 공백`은 코드의 구조를 시각적으로 일관되게 드러내는 규칙입니다. 포맷 논쟁을 줄이고 리뷰가 의미 있는 내용에 집중하도록 돕습니다.
...
쉽게 말해, `세로 공백` 규칙은 독자가 코드를 읽으면서 추측해야 하는 부분을 줄이기 위한 안전장치입니다. 작성자는 배경을 알고 있지만, 리뷰어와 미래의 유지보수자는 이름·선언·형식에 남은 단서만 보고 판단합니다.
