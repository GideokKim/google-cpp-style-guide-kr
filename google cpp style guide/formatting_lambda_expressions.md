...
`람다 표현식 형식`은 짧은 지역 동작을 가까이에 둘 수 있게 하지만 캡처와 반환 타입이 숨은 상태를 만들 수 있음을 경계하는 규칙입니다. 캡처 목록은 의도를 드러내야 합니다.
...
쉽게 말해, `람다 표현식 형식` 규칙은 독자가 코드를 읽으면서 추측해야 하는 부분을 줄이기 위한 안전장치입니다. 작성자는 배경을 알고 있지만, 리뷰어와 미래의 유지보수자는 이름·선언·형식에 남은 단서만 보고 판단합니다.
