...
`부동소수점 리터럴`은 부동소수점 표현의 정밀도와 비교 방식이 직관과 다를 수 있음을 드러내는 규칙입니다. 리터럴과 타입 선택은 계산 의도를 보여 주어야 합니다.
...
쉽게 말해, `부동소수점 리터럴` 규칙은 독자가 코드를 읽으면서 추측해야 하는 부분을 줄이기 위한 안전장치입니다. 작성자는 배경을 알고 있지만, 리뷰어와 미래의 유지보수자는 이름·선언·형식에 남은 단서만 보고 판단합니다.
