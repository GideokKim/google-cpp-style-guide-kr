...
`생성자 초기화 목록`은 객체 생성 시점에 과도한 일을 숨기지 말라는 규칙입니다. 생성자는 불변식을 세우는 데 집중하고 실패 가능하거나 무거운 작업은 더 명시적인 경로로 분리하세요.
...
쉽게 말해, `생성자 초기화 목록` 규칙은 독자가 코드를 읽으면서 추측해야 하는 부분을 줄이기 위한 안전장치입니다. 작성자는 배경을 알고 있지만, 리뷰어와 미래의 유지보수자는 이름·선언·형식에 남은 단서만 보고 판단합니다.
