...
가능한 한 전방 선언을 피하세요. 대신 [필요한 헤더를 포함하세요](include_what_you_use.md).

**정의:**

"전방 선언(forward declaration)"은 대응하는 정의 없이 엔터티만 선언하는 것입니다.
...
여러분의 프로젝트가 소유하지 않은 엔터티는 전방 선언하지 마세요.

여러분이 소유한 엔터티에 대해서는 다음과 같이 하세요.

- 선언이 담긴 헤더를 직접 포함해도 부담이 없을 만큼 가볍게 만들려고 해 보세요.
- 전체 헤더를 포함하는 비용이 너무 크다면, 그 헤더의 컴파일 속도를 높여 보세요.
- 그럼에도 전방 선언이 느려짐을 해결하는 더 나은 방법이라는 것이 여러분의 최선의 공학적 판단이라면, 그 판단에 따라 전방 선언을 사용하세요.
