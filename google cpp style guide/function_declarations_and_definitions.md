...
이 규칙의 핵심은 문맥상 명백히 사용되지 않는 매개변수는 이름을 생략할 수 있습니다: 명확하지 않을 수 있는 사용되지 않는 매개변수는 함수 정의에서 변수 이름을 주석 처리해야 합니다.

실제로 코드를 볼 때는 - 매개변수 이름은 함수 정의에 매개변수가 사용되지 않는 경우에만 생략할 수 있습니다.

점검할 때는 특히 다음을 확인하세요:

- 기능은 다음과 같습니다: 한 줄에 들어갈 텍스트가 너무 많은 경우: 또는 첫 번째 매개변수조차 맞출 수 없는 경우: 참고할 사항: - 좋은 매개변수 이름을 선택하세요.
- 함수 이름과 같은 줄에 반환 유형이 있고, 맞는 경우 매개변수가 같은 줄에 있습니다.
