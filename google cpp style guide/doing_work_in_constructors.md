...
이 규칙의 핵심은 클래스가 현재 하위 클래스로 분류되지 않은 경우에도 나중에 클래스를 수정하면 이 문제가 조용히 발생하여 많은 혼란을 야기할 수 있습니다.

실제로 코드를 볼 때는 - 프로그램 충돌(항상 적절한 것은 아님)이나 예외 사용(금지됨)을 제외하고 생성자가 오류를 알리는 쉬운 방법은 없습니다.

점검할 때는 특히 다음을 확인하세요:

- - 작업이 가상 함수를 호출하는 경우 이러한 호출은 하위 클래스 구현으로 전달되지 않습니다.
- 호출할 수 있는 공용 메서드에 영향을 주는 다른 상태가 없는 개체에서는 Init() 메서드를 사용하지 마세요(이 형식의 반구조화된 개체는 특히 올바르게 작업하기 어렵습니다).
