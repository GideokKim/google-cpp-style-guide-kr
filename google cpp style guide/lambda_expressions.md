...
- 람다가 현재 범위를 벗어날 수 있는 경우, 참조 기본 캡처(`[&]`)보다 변수를 콕 집어 캡처(`[&foo]`)하는 명시적 캡처를 선호합니다. 그래야 무엇을 참조로 잡는지 드러나고, 멤버 함수를 캡처하면 컴파일 오류가 나서 위험을 조기에 알 수 있습니다. (구체적인 예시는 아래 "옮긴이 풀이" 참조.)
